public class Hogwarts {
    ...
    public void printPowerDifference(Hogwarts student) {
        if (this.getPower() > student.getPower()) {
            System.out.printf("%s обладает бОльшой мощностью магии, чем %s", this.name, student.name);
        } else if (this.getPower() < student.getPower()) {
            System.out.printf("%s обладает бОльшой мощностью магии, чем %s", student.name, this.name);
        } else {
    ...
    public String toString() {
        return "Имя ученика:" + this.name +"\nСила магии: " + this.powerOfMagic+
                "\nРасстоянии трансгрессии " + this.transgressionDistance;
    }
